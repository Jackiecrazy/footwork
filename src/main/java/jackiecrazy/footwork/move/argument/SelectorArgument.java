...
public class SelectorArgument implements Argument<List<Entity>> {
    ...
    private Argument<Double> limit = FixedNumberArgument.ZERO;
    ...
    public List<Entity> resolve(ArgumentContext argumentContext) {
        ...
        if (shape == CircleEnums.SWEEPTYPE.NONE) {
            if (GeneralUtils.getDistSqCompensated(argumentContext.target(), pos) < ra * ra)
                resolved.add(argumentContext.target());
            return resolved;
        }
        ...
        switch (sort) {
            case nearest -> resolved.sort((a, b) -> (int) ((a.distanceToSqr(pos) - b.distanceToSqr(pos)) * 100));
            case furthest -> resolved.sort((a, b) -> (int) ((b.distanceToSqr(pos) - a.distanceToSqr(pos)) * 100));
            case random -> resolved.sort((a, b) -> Footwork.rand.nextInt(3)-1);
        }
        return resolved;
    }
