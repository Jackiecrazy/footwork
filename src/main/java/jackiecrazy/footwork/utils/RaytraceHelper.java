...
public class RaytraceHelper {
    ...
    public static RaytraceHelper INSTANCE=new RaytraceHelper();

    /**
     * @author Vazkii
     */
    public Entity getEntityLookedAt(Entity e, double finalDistance) {
        return getEntityLookedAt(e, e.getEyePosition(), e.getLookAngle(), finalDistance);
    }

    public Entity getCameraMouseOverEntity(Minecraft mc, double finalDistance) {
        Entity attempt1 = getEntityLookedAt(mc.player, mc.gameRenderer.getMainCamera().getPosition(), new Vec3(mc.gameRenderer.getMainCamera().getLookVector()), finalDistance);
        Entity attempt2 = getEntityLookedAt(mc.player, mc.gameRenderer.getMainCamera().getPosition(), mc.player.getLookAngle(), finalDistance);
        return attempt1 == null ? attempt2 : attempt1;
    }

    public HitResult raycastCamera(Minecraft mc, double finalDist) {
        return raycast(mc.gameRenderer.getMainCamera().getPosition(), new Vec3(mc.gameRenderer.getMainCamera().getLookVector()), mc.player, finalDist);
    }

    /**
     * @author Vazkii
     */
    public Entity getEntityLookedAt(Entity e, Vec3 position, Vec3 look, double finalDistance) {
        Entity foundEntity = null;
        double distance = finalDistance;
        HitResult pos = raycast(position, look, e, finalDistance);
    ...
    public HitResult raycast(Entity e, double len) {
    ...
    public HitResult raycast(Vec3 origin, Vec3 ray, Entity e, double len) {
